	...
	<div class="reg">
		<div class="content">
		  ...
		  <div class="login-box">
		    <p class="lsolid"></p>
		    <div class="login">
		      <router-link to="/login"> <a href="jacascript:;">登录</a></router-link>
		      <span></span>
		      <router-link to="/reg"> <a href="jacascript:;">注册</a></router-link>
		    </div>
		    <p class="rsolid"></p>
		  </div>
		  <ul>
		   <uc-input v-model="username" label="账号"></uc-input>
		   <uc-input v-model="password" label="密码" type="password"></uc-input>
		  </ul>
		  <div class="footbox">
		    <uc-button @click="reg" style="margin-top:.23rem">注册</uc-button>
		   <a href="javascript:;" class="tierros">{{mess}}</a>
		  </div>
		</div>
	...
	methods:{
		reg(){
			this.$axios({
			   ...
			   res=>{
				   if(this.username =='lin' && this.password =='lin123'){
...
<style scoped>
  ...
  .content .tierros{width:2rem; font-size:0.25rem; margin-top:0.28rem; display:block;}
</style>
